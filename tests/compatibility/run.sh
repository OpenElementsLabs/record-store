...
export OES_MANAGEMENT_SYSTEM_TOKEN="oes-compat-management-token-at-least-thirty-two-bytes"
export OES_STORAGE_DATA_DIRECTORY="$run_directory/data"
export OES_STORAGE_ENCRYPTION_ENABLED="true"
export OES_COMPAT_ENDPOINT="http://127.0.0.1:7600"
...
for _ in {1..100}; do
  if curl --fail --silent "http://127.0.0.1:7601/ready" >/dev/null; then
    break
  fi
...
curl --fail --silent "http://127.0.0.1:7601/ready" >/dev/null
