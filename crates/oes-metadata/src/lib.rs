...
impl MetadataRepository for RedbMetadataRepository {
    async fn create_bucket(&self, bucket: &Bucket) -> Result<(), MetadataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let write = database
                .begin_write()
                .map_err(|e| backend("begin create bucket", e))?;
            ...
            write.commit().map_err(|e| backend("commit bucket", e))
        })
        .await?
    }
    ...
    async fn set_bucket_versioning(
        ...
        self.update_bucket(id, move |bucket| {
            if bucket.versioning == VersioningState::Enabled && state == VersioningState::Disabled {
    ...
    async fn set_bucket_quota(
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin quota update", e))?;
            let usage = read_bucket_usage(&write, id)?;
            if !quota
                .bytes
            ...
            let bucket = update_bucket_tx(&write, id, |bucket| {
                bucket.quota = quota;
                Ok(())
            })?;
            write
                .commit()
                .map_err(|e| backend("commit quota update", e))?;
            Ok(bucket)
        })
        .await?
    }
    ...
    async fn delete_bucket(&self, name: &BucketName) -> Result<Bucket, MetadataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin delete bucket", e))?;
            let bucket: Bucket = {
                let table = write
                    .open_table(BUCKET_NAMES)
                    .map_err(|e| backend("open bucket names", e))?;
            ...
            if read_bucket_usage(&write, bucket.id)?.versions != 0
                || has_multipart(&write, bucket.id)?
            {
            ...
            write
                .commit()
                .map_err(|e| backend("commit delete bucket", e))?;
            Ok(bucket)
        })
        .await?
    }
    ...
    async fn put_object(
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin object commit", e))?;
            let bucket =
                read_bucket(&write, metadata.bucket_id)?.ok_or(MetadataError::BucketNotFound)?;
            let key = object_key(metadata.bucket_id, &metadata.key);
            let previous: Option<ObjectMetadata> =
                read_tx(&write, OBJECTS, &key, "read current object")?;
            ...
            write.commit().map_err(|e| backend("commit object", e))?;
    ...
    async fn delete_object(
        ...
        let db = Arc::clone(&self.database);
        let object_key_value = key.clone();
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin object delete", e))?;
            let bucket_record =
                read_bucket(&write, bucket)?.ok_or(MetadataError::BucketNotFound)?;
            let key = object_key(bucket, &object_key_value);
            let current: Option<ObjectMetadata> =
                read_tx(&write, OBJECTS, &key, "read current object")?;
            ...
            write
                .commit()
                .map_err(|e| backend("commit object delete", e))?;
            Ok(result)
        })
        .await?
    }
    ...
    async fn delete_object_version(
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin version delete", e))?;
            ...
            write
                .commit()
                .map_err(|e| backend("commit version delete", e))?;
    ...
    async fn create_multipart_upload(&self, upload: &MultipartUpload) -> Result<(), MetadataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin multipart", e))?;
            ...
            write.commit().map_err(|e| backend("commit multipart", e))
        })
        .await?
    }
    ...
    async fn put_multipart_part(
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin part commit", e))?;
            ...
            let bucket =
                read_bucket(&write, upload.bucket_id)?.ok_or(MetadataError::BucketNotFound)?;
            ...
            write.commit().map_err(|e| backend("commit part", e))?;
            Ok(previous)
        })
        .await?
    }
    ...
    async fn begin_multipart_completion(
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin completion", e))?;
            ...
            write
                .commit()
                .map_err(|e| backend("commit completion state", e))?;
            Ok(upload)
        })
        .await?
    }
    ...
    async fn finish_multipart_upload(
        ...
        remove_multipart(&self.database, id, true).await
    }
    async fn abort_multipart_upload(
        &self,
        id: UploadId,
    ) -> Result<MultipartCleanupResult, MetadataError> {
        remove_multipart(&self.database, id, false).await
    }
    ...
    async fn recover_multipart_completions(&self) -> Result<MultipartCleanupResult, MetadataError> {
        ...
        let uploads = tokio::task::spawn_blocking(move || {
            let read = db
                .begin_read()
                .map_err(|e| backend("begin completion recovery", e))?;
            let table = read
                .open_table(MULTIPART)
                .map_err(|e| backend("open multipart", e))?;
            let mut out = Vec::new();
        ...
        for upload in uploads {
            ...
            let committed = self
                .get_object(upload.bucket_id, &upload.key)
                .await?
                .is_some_and(|metadata| metadata.id == object_id);
            if committed {
                cleaned.extend(self.finish_multipart_upload(upload.id).await?.parts);
            } else {
                let db = Arc::clone(&self.database);
                tokio::task::spawn_blocking(move || {
                    let write = db
                        .begin_write()
                        .map_err(|e| backend("begin reset completion", e))?;
                    ...
                    write
                        .commit()
                        .map_err(|e| backend("commit reset completion", e))
                })
                .await??;
            }
        }
    ...
    async fn put_lifecycle_rule(&self, rule: &LifecycleRule) -> Result<(), MetadataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin lifecycle rule", e))?;
            ...
            write
                .commit()
                .map_err(|e| backend("commit lifecycle rule", e))
        })
        .await?
    }
    ...
    async fn delete_lifecycle_rule(&self, id: LifecycleRuleId) -> Result<(), MetadataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin lifecycle rule deletion", e))?;
            {
                let mut table = write
                    .open_table(LIFECYCLE_RULES)
                    .map_err(|e| backend("open lifecycle rules", e))?;
            ...
            write
                .commit()
                .map_err(|e| backend("commit lifecycle rule deletion", e))
        })
        .await?
    }
    ...
    async fn complete_cleanup(&self, id: ObjectId) -> Result<(), MetadataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin cleanup completion", e))?;
            {
                let mut table = write
                    .open_table(CLEANUP)
                    .map_err(|e| backend("open cleanup", e))?;
            ...
            write.commit().map_err(|e| backend("commit cleanup", e))
        })
        .await?
    }
...
impl RedbMetadataRepository {
    async fn update_bucket<F>(&self, id: BucketId, update: F) -> Result<Bucket, MetadataError>
    where
        F: FnOnce(&mut Bucket) -> Result<(), MetadataError> + Send + 'static,
    {
        let db = Arc::clone(&self.database);
        tokio::task::spawn_blocking(move || {
            let write = db
                .begin_write()
                .map_err(|e| backend("begin bucket update", e))?;
            let bucket = update_bucket_tx(&write, id, update)?;
            write
                .commit()
                .map_err(|e| backend("commit bucket update", e))?;
            Ok(bucket)
        })
        .await?
    }
...
async fn remove_multipart(
    ...
    tokio::task::spawn_blocking(move || {
        let write = db
            .begin_write()
            .map_err(|e| backend("begin multipart removal", e))?;
        let upload: MultipartUpload = read_tx(
            &write,
            MULTIPART,
            id.as_uuid().as_bytes().as_slice(),
        ...
        write
            .commit()
            .map_err(|e| backend("commit multipart removal", e))?;
...
mod tests {
    ...
    async fn versions_markers_and_restart_are_durable() {
        ...
        repo.delete_object(bucket.id, &first.key)
            .await
            .expect("delete");
        drop(repo);
